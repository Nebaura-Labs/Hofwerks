...
// Match crosshair motion for smoother/lazier horizontal glide
...
export function ChartTooltip({
	...
	const visibleDots = useMemo(() => {
		...
		return lines
			...
			.filter((dot): dot is { color: string; key: string; x: number; y: number } =>
				Boolean(dot)
			);
	}, [tooltipData, isHorizontal, lines, x]);
	...
	const title = useMemo(() => {
		...
		// For vertical charts, use time
		return formatChartTime(xAccessor(tooltipData.point));
	...
	const tooltipContent = (
		<>
			...
			{showDots && visible && !isHorizontal && visibleDots.length > 0 && (
				<svg
					aria-hidden="true"
					className="pointer-events-none absolute inset-0"
					height="100%"
