...
fn verify_serial_port(port_name: String, baud_rate: u32) -> Result<bool, String> {
	#[cfg(target_os = "ios")]
	{
		...
		Err(String::from(
			"Serial hardware verification is not supported on iOS yet.",
		))
	}
	...
	let thread = if mode == "simulator" {
		...
		#[cfg(target_os = "ios")]
		{
			...
			return Err(String::from(
				"Hardware serial mode is not supported on iOS yet. Use simulator mode.",
			));
		}
...
pub fn run() {
	tauri::Builder::default()
		.setup(|_app| {
			#[cfg(target_os = "ios")]
			configure_ios_webview_insets(_app);
			Ok(())
		})
...
#[cfg(target_os = "ios")]
fn configure_ios_webview_insets(app: &tauri::App) {
	...
	let _ = main_window.with_webview(|platform_webview| unsafe {
		...
		let scroll_view_ptr: *mut AnyObject = msg_send![webview, scrollView];
		if scroll_view_ptr.is_null() {
			return;
		}

		let scroll_view = &*scroll_view_ptr;
		// UIScrollViewContentInsetAdjustmentNever = 2
		let _: () = msg_send![scroll_view, setContentInsetAdjustmentBehavior: 2isize];
		let _: () = msg_send![scroll_view, setAutomaticallyAdjustsScrollIndicatorInsets: false];
	});
}
